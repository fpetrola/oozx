...
public class ConditionalsTransformInstructionsTest extends TransformInstructionsTest {
  @Inject
...
//    byteCodeGenerator.generate(() -> ClassMaker.beginExternal("JSW").public_(), "JSW.class");
  ...
  public void testJRNZSimpleLoopJumpingToBegining() {
    ...
    step();
    assertEquals(1, r(PC).read());
    step(2);

    assertLoop.run();

    assertEquals(7, r(PC).read());
    step();
    assertEquals(1, r(PC).read());
    step();
  ...
  public void stackOverflowBug() {
    ...
    List<Instruction> executedInstructions = getExecutedInstructions();
    executedInstructions.size();
  }
  ...
  public void testLddr() {
    ...
    add(new Lddr(r(PC), rp(BC), new Ldd(r(DE), rp(BC), rp(HL), f(), mem(), new MockedIO(), r(A))));
