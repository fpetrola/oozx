...
//        System.out.println(String.format("Press keyCode = %d, modifiers = %d", key, evt.getModifiersEx()));
    ...
    /*
     * Windows never sends VK_ALT_GRAPH; it sends Ctrl then Alt instead, in that order. The Ctrl
     * arrives as a normal press with the Alt already showing both CTRL and ALT as active
     * modifiers, and its side effect is "pressing" Symbol-Shift, which then has to be undone.
     * On some Windows keyboards AltGr shows up as a right Alt instead of the dedicated key.
     */
    if (winBug && key == KeyEvent.VK_ALT && (evt.getModifiersEx() == (InputEvent.CTRL_DOWN_MASK | InputEvent.ALT_DOWN_MASK)
      ...
      // On Windows, Alt+Control together most likely means AltGr was actually pressed.
...
//        System.out.println(String.format("Release keyCode = %d, modifiers = %d", key, evt.getModifiersEx()));
    ...
    /*
     * As above (key-release path): Windows sends Ctrl then Alt for AltGr, the Ctrl release
     * carries the Alt modifier, and a plain Alt release is skipped so shortcuts keep working
     * without a spurious emulator keypress.
     */
