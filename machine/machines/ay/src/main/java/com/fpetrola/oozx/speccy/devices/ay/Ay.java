 ...
 * samples.
 * <p>
 * Each channel could have its own synth, needed apart only to put two of them left and
...
class Ay implements AudioSource {
  ...
  /** As published for the chip, scaled to a full sixteen bits. */
  ...
  private void synthesise(long frameTstates) {
    ...
    for (long f = 0; f < frameTstates; f += STEP) {
      ...
      // something happens. Taking every one of the two thousand steps a frame
      // has gives the same samples.
