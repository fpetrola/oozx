...
La biblioteca de referencia está instalada en esta máquina como
...
`model.tests.formats.ReferenceOracle` recorre los bloques de una cinta con la referencia;
...
bloques que la referencia, en el mismo orden, incluidos los difíciles: datos generalizados, nivel de
...
2. En un tap, Java no le pone id a los bloques; la referencia dice que son `0x10`, datos estándar.
3. Java no valida: los seis archivos que la referencia rechaza por estar truncados o mal formados,
...
la referencia, en el mismo orden y con los mismos ids, y cinco de los seis inválidos se rechazan
...
un objeto Java. Así que los dos se escriben de vuelta con el mismo escritor, el de la referencia: la
...
2. **`empty.szx` lee A y F al revés.** Y la causa es justo el tipo de caso que no se testea a
   mano: las versiones de la referencia hasta la 0.5.0 escribían esos dos registros invertidos en un
   szx, así que ella misma mira el bloque de creador y, si dice `libspectrum: 0.5.0` o
...
3. **A y F al revés en un szx viejo**, y también el par alternativo. Se lee el bloque de creador y,
   si dice `libspectrum: 0.5.0` o anterior, se dan vuelta los dos pares, que es lo que hace ella
   misma.
...
No hace falta JNA ni tener la biblioteca instalada: la grabación es un archivo del repo.
...
3. Encima de eso, los dos oráculos que no necesitan la biblioteca: la reproducción de una RZX, que
