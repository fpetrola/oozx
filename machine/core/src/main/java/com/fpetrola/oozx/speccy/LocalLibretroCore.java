...
import com.fpetrola.oozx.speccy.modules.BeanPosition;
...
//    return speccy.memory.mapWrite[i];
  ...
  public int retro_get_memory_map_read_source(int index) {
    return speccy.memory.mapRead[index].source;
  }

  @Override
  public int retro_get_memory_map_write_source(int index) {
    return speccy.memory.mapWrite[index].source;
  }

  @Override
  public int retro_get_memory_map_read_page_num(int index) {
    return speccy.memory.mapRead[index].getPageNum();
  }

  @Override
  public int retro_get_memory_map_write_page_num(int index) {
    return speccy.memory.mapWrite[index].getPageNum();
  }

  @Override
  public int retro_get_current_screen() {
    return speccy.memory.currentScreen;
  }

  @Override
  public int retro_ram_locked() {
    return speccy.machine.current.getRamInfo().locked ? 1 : 0;
  }

  @Override
  public int retro_get_ula_contention(int i) {
    return speccy.ula.contention[i];
  }

  @Override
  public void retro_set_late_timings(int b) {
    speccy.settings.current.lateTimings = b != 0;
  }

  @Override
  public int retro_get_late_timings() {
    return speccy.settings.current.lateTimings ? 1 : 0;
  }
