 ...
 * <p>
 * Keys go through {@link com.fpetrola.oozx.speccy.modules.Keyboard}. Writing the key code into
...
public class TapeAutoLoader {
  ...
  private void press(KeyboardKeyName... keys) {
    steps.add(() -> {
      for (KeyboardKeyName key : keys) {
        speccy.keyboard.press(key);
      }
      framesToWait = KEY_HOLD_FRAMES;
    });
    steps.add(() -> {
      for (KeyboardKeyName key : keys) {
        speccy.keyboard.release(key);
      }
  ...
  private void startTape() {
    ...
    if (!speccy.tape.insert(tapeFile)) {
      error = "the tape deck rejected " + tapeFile;
    } else if (!speccy.tape.play(false)) {
  ...
  public void step() {
    ...
    if (waitingForTapeToStop) {
      if (!speccy.tape.isTapePlaying()) {
        speccy.z80.changeSpeed(speedAfterLoading);
