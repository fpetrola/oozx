...
package com.fpetrola.oozx.speccy.modules.sound.blip;

import com.fpetrola.oozx.speccy.modules.sound.Colouring;
import static com.fpetrola.oozx.speccy.modules.sound.blip.BlipBuffer.*;
...
public class BlipSynth {
  ...
  public BlipSynth(int quality, long soundFreq, int msec, long effectiveSpeed, Colouring colouring, double volume) {
    ...
    lastAmp = 0;
    blipBuffer.bassFreq(colouring.bass());
    trebleEq(new BlipEq(colouring.treble()));
  }
  ...
  private void trebleEq(BlipEq eq) {
    ...
    double base_unit = 32768.0; // scale kernelUnit is built at, so volume() dividing by BLIP_UNSCALED lands in range
  ...
  private void volumeUnit(double newUnit) {
    ...
    if (factor > 0.0) {
      ...
      // a very small unit needs the kernel scaled down too, or it rounds to zero
      while (factor < 2.0) {
        shift++;
        factor *= 2.0;
      }
      ...
      if (shift != 0) {
        // biased into positive range first: a negative value's sign-preserving right shift
        // rounds toward zero, which this must not do
