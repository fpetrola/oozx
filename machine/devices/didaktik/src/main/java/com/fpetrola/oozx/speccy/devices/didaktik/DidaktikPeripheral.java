 ...
 * that is not emulated, and a SNAP button that takes an NMI into its ROM by turning the
 * instruction at 0x0066 into RST 0. Pages in at 0x0000 and 0x0008, out at 0x1700.
 */
@Singleton
public class DidaktikPeripheral extends WdDiskInterface {
  ...
  public DidaktikPeripheral(MemoryBus memory, Cpu cpu, Fdd.Limits floppy, Roms roms, Scheduler events, Machine machine) {
    ...
    // READY comes from the board and is always there.
    ...
    ports(fdcRegister(0x81, FdcRegister.STATUS_COMMAND),
        ...
        Wired.at(0x0080, 0x0000, new DefaultPortHandler(true, true) {
          public BusAnswer read(int port) {
            // The 8255: answers all ones and takes nothing.
  ...
  /** The Didaktik 80, a disk interface with its own ROM. */
}
