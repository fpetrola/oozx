...
public class MouseInternalFrame extends DeviceFrame<KempstonMousePeripheral> {
   ...
   * <p>
   * The pointer is grabbed and warped back to the middle after every
   * movement. Without it the hand runs
  ...
  /** Plugged into that machine's expansion port, or taken out of it. */
  @Override
  ...
  private void report(MouseEvent where) {
    ...
    if (held) {
      // The movement is measured from the middle, and then the pointer is put
  ...
  private void button(MouseEvent which, boolean down) {
    // The middle button lets go, and takes hold again: it is the
