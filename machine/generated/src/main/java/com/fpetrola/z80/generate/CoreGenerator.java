...
import com.fpetrola.z80.tstates.Contention;
...
public class CoreGenerator {
  /** What is being generated: a core made against a machine, which holds that machine's objects and reaches its memory without a call. */
  public static class Target {
    ...
    /** Whose contend is written into the class, one method per pair of cycles it takes. */
    public Object contention;
    /**
     * How many opcodes share a method: the cases are split so HotSpot compiles them at all, a
     * single method for all of them being past the size where one is ever compiled.
     * <p>
     * Where the best split is, if there is one, is not known. Four, eight, sixteen and thirty two
     * opcodes a method were measured on 2026-09-11 with a fixed program and instructions a second,
     * and came out the same: the number that used to be here - eight being four percent over
     * sixteen and well over thirty two - was read off frames a second while a ROM booted, which
     * measures what the ROM is doing that frame as much as the emulator, and swings 70 percent on
     * one unchanged build. See InstructionsPerSecond.
     */
    public int groupShift = Integer.getInteger("oozx.groupshift", 3);

    public Target(String packageName, String className) {
  ...
  /** The instruction graph the generator reads: the model's opcode tables, built over the state. */
  ...
  public CoreGenerator(State state, SourceIndex index, Target target) {
    this.state = state;
    this.root = tables(state);
    this.target = target;
    this.spec = new Specializer(index);
    this.memptrUpdater = spec.of(new MemptrUpdater(state.getMemptr(), state.getMemory()));
    this.contention = new RecordingPhaseProcessor(state, event -> {
    });
  }
  ...
  public String generate() {
    ...
    imports.addAll(List.of("com.fpetrola.z80.registers.UnrolledRegisterBank", "com.fpetrola.z80.generate.GeneratedCore", "com.fpetrola.z80.cpu.State", "com.fpetrola.z80.cpu.IO"));
    for (Class<?> type : target.held.values()) {
      Class<?> named = type;
      while (named.isArray())
        named = named.getComponentType();
      if (!named.isPrimitive())
        imports.add(named.getName().replace('$', '.'));
    }
    for (String i : imports)
      if (i.contains(".") && (!i.startsWith(target.packageName + ".") || i.indexOf('.', target.packageName.length() + 1) >= 0))
    ...
    out.append("public class ").append(target.className).append(" extends UnrolledRegisterBank implements GeneratedCore {\n");
