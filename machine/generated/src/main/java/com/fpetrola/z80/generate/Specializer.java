...
public class Specializer {
  ...
  private Object dispatch(Obj receiver, String name, List<Object> args, List<Statement> out, Scope caller) {
    ...
    if (receiver.expression != null && opaque.contains(name))
      return answeredAtRunTime(receiver, name, args);
    if (value == null && receiver.expression != null) {
      if (receiver.runtimeClass().isSealed())
        return devirtualize(receiver, name, args, out);
      // A final class answers for itself, so its method is its body, inlined over the object.
      Found own = java.lang.reflect.Modifier.isFinal(receiver.runtimeClass().getModifiers()) ? findMethod(receiver.runtimeClass(), name, args.size()) : null;
      if (own != null && own.method.getBody().isPresent())
        return inline(receiver, own, args, out);
      // Known by type only, found at run time - a device's registers in a page table: the call is
      // written as it is, and the machine running decides who answers it.
      return answeredAtRunTime(receiver, name, args);
    }
  ...
  /** The call as it is, and what it answers as something reached at run time - unless it is a value, which is just the expression. */
  private Object answeredAtRunTime(Obj receiver, String name, List<Object> args) {
    MethodCallExpr call = callAsIs(receiver.expression, name, args);
    Class<?> answer = returnTypeOf(receiver.runtimeClass(), name, args.size());
    return answer == void.class || isValueType(answer) ? call : atRunTime(null, call, answer);
  }

  private static Class<?> returnTypeOf(Class<?> c, String name, int arity) {
    Method m = findReflected(c, name, arity);
    if (m == null)
      throw new UnsupportedOperationException("no " + name + "/" + arity + " on " + c.getSimpleName());
    return m.getReturnType();
  }

  private MethodCallExpr callAsIs(Expression receiver, String name, List<Object> args) {
    NodeList<Expression> arguments = new NodeList<>();
    for (Object a : args)
      arguments.add(asExpression(a));
    return new MethodCallExpr(receiver.clone(), name, arguments);
  }

  /**
   * A call on an object known by a sealed type only - which memory covers a page is decided as
   * the machine runs, but what kinds of memory there are is not. The polymorphism is what goes:
   * one branch per implementation of the method, with the kinds that share it asked for by
   * instanceof, and the implementation's body inlined over the object cast to it. The model
   * keeps its classes; the generated code keeps their code.
   */
  private Object devirtualize(Obj receiver, String name, List<Object> args, List<Statement> out) {
    Map<Class<?>, List<Class<?>>> kindsByImplementation = new LinkedHashMap<>();
    for (Class<?> kind : kinds(receiver.runtimeClass(), name, args.size())) {
      Found found = findMethod(kind, name, args.size());
      Class<?> implementation = found == null || found.method.getBody().isEmpty() ? kind : found.declaring;
      kindsByImplementation.computeIfAbsent(implementation, k -> new ArrayList<>()).add(kind);
    }
    List<Map.Entry<Class<?>, List<Class<?>>>> branches = new ArrayList<>(kindsByImplementation.entrySet());
    Found any = findMethod(branches.get(0).getValue().get(0), name, args.size());
    boolean isVoid = any == null || any.method.getType().isVoidType();
    String result = isVoid ? null : fresh(name);
    if (result != null)
      out.add(declare(any.method.getType(), result, null));
    Statement chain = null;
    for (int i = branches.size() - 1; i >= 0; i--) {
      Class<?> implementation = branches.get(i).getKey();
      List<Class<?>> kinds = branches.get(i).getValue();
      List<Statement> body = new ArrayList<>();
      Expression self = receiver.expression.clone();
      if (implementation != receiver.runtimeClass()) {
        imports.add(implementation.getName().replace('$', '.'));
        self = new EnclosedExpr(new CastExpr(new ClassOrInterfaceType(null, implementation.getSimpleName()), self));
      }
      Found found = findMethod(implementation, name, args.size());
      Object value = found == null || found.method.getBody().isEmpty()
          ? callAsIs(self, name, args)
          : inline(atRunTime(null, self, implementation), found, args, body);
      if (result != null)
        body.add(new ExpressionStmt(new AssignExpr(new NameExpr(result), asExpression(value), AssignExpr.Operator.ASSIGN)));
      else if (value instanceof Expression e && hasEffect(e))
        body.add(new ExpressionStmt(e));
      if (chain == null) {
        chain = block(body);
      } else {
        Expression condition = null;
        for (Class<?> kind : kinds) {
          imports.add(kind.getName().replace('$', '.'));
          Expression test = new InstanceOfExpr(receiver.expression.clone(), new ClassOrInterfaceType(null, kind.getSimpleName()));
          condition = condition == null ? test : new BinaryExpr(condition, test, BinaryExpr.Operator.OR);
        }
        chain = new IfStmt(condition, block(body), chain);
      }
    }
    out.add(chain);
    return result == null ? null : new NameExpr(result);
  }

  /**
   * The kinds a sealed type comes in, for that method: down to the ones that are not sealed
   * further, except that a sealed kind none of whose kinds answers the method differently is one
   * kind - a ROM and a RAM read alike, and are told apart by one test, not two.
   */
  private List<Class<?>> kinds(Class<?> sealed, String name, int arity) {
    List<Class<?>> kinds = new ArrayList<>();
    for (Class<?> permitted : sealed.getPermittedSubclasses()) {
      if (permitted.isSealed() && kinds(permitted, name, arity).stream().anyMatch(kind -> overridesBelow(kind, permitted, name, arity)))
        kinds.addAll(kinds(permitted, name, arity));
      else
        kinds.add(permitted);
    }
    return kinds;
  ...
  private boolean overridesBelow(Class<?> kind, Class<?> ancestor, String name, int arity) {
    Found found = findMethod(kind, name, arity);
    return found != null && found.declaring != ancestor && ancestor.isAssignableFrom(found.declaring) && found.declaring != ancestor;
  }
  ...
  /**
   * What stays a call instead of being inlined, and the name of the field it is called on. A core
   * generated against a machine takes the memory off this list and inlines it, keeping only what
   * is really opaque.
   */
  public final Map<Class<?>, String> terminals = new LinkedHashMap<>(Map.of(Memory.class, "memory", IO.class, "io", State.class, "state"));
  /** Methods written as the call they are, on an object held by name: what they answer is decided as the machine runs. */
  public final Set<String> opaque = new HashSet<>();

  /** What an import names of an array type: the class of its elements, however deep. */
  static Class<?> elementOf(Class<?> type) {
    while (type.isArray())
          ...
          if (d.getInitializer().isPresent()) {
            ...
            if (init instanceof Obj o) {
              ...
              if (o.expression != null && !(o.expression instanceof NameExpr)) {
                imports.add(elementOf(o.runtimeClass()).getName().replace('$', '.'));
  ...
  Object rewriteExpr(Expression e, Scope scope, List<Statement> out) {
    ...
    if (e instanceof BinaryExpr b) {
      ...
      if (left instanceof Obj || right instanceof Obj) {
        // An object reached at run time is compared at run time: what is there now says nothing
        // about what will be there then - a page table entry, say.
        boolean atRunTime = (left instanceof Obj lo && lo.expression != null && !lo.isVirtual()) || (right instanceof Obj ro && ro.expression != null && !ro.isVirtual());
        if (atRunTime)
          return Folder.fold(new BinaryExpr(asExpression(left), asExpression(right), b.getOperator()));
  ...
  Expression asExpression(Object o) {
    ...
    if (o instanceof Obj obj && obj.value instanceof Enum<?> constant)
      return new FieldAccessExpr(new NameExpr(constant.getDeclaringClass().getCanonicalName()), constant.name());
    if (o == null)
