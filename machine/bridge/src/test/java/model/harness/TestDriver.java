...
//    commandHandler.addNoResultCommand(new StepCommand());
  ...
  /** A slot of Fuse's memory map: where its bytes come from, and which page of that. */
  public record MemoryMapEntry(int getSource, int getPageNum) {
  }

  public MemoryMapEntry getMemoryMapRead(int i) {
    return (MemoryMapEntry) commandHandler.executeCommand(new GetMemoryMapRead(i));
  }

  public MemoryMapEntry getMemoryMapWrite(int i) {
    return (MemoryMapEntry) commandHandler.executeCommand(new GetMemoryMapWrite(i));
  }
