...
public class RzxSession {
  ...
  private RzxSession(RzxFile recording, Speccy speccy, RzxPlayback playback, SwitchableIO ports) {
    ...
    // A playback holds the processor it drives, so it is made again over whichever one the
    // machine moves to. The player is kept: it is what holds the place in the recording.
    speccy.processors.onChanged(() -> this.playback = playbackOf(speccy, ports.player, recording));
    this.ports = ports;
  }

  /** A driver for this recording over this machine: it steps the machine, and ends the machine's frames where the recording ends its own. */
  private static RzxPlayback playbackOf(Speccy speccy, RZXPlayerIO player, RzxFile recording) {
    return new RzxPlayback(speccy.cpu.getOoz80(), player, recording,
        length -> speccy.machine.current.frameEnded(length), speccy.cpu::step);
  }
  ...
  private static class SwitchableIO implements IO {
    ...
    @Inject
    private volatile PeripheralIO hardware;
  ...
  public static RzxSession open(File file) {
    ...
    // Made the way every machine is made, with the ports overridden: made from the bare module
    // instead, it skipped the settings, and the sound stayed switched off - a replay was silent.
    Speccy speccy = Speccy.create(new SpectrumZ80Clock(), new AbstractModule() {
      @Override
      protected void configure() {
        bind(IO.class).toInstance(ports);
        // The hardware ports go in here, BEFORE the snapshot, because loading one WRITES: it puts
        // back the paging the snapshot was saved under by sending it through the machine's own
        // port. Set afterwards, that write went to a null and was dropped without a word - the
        // machine kept bank 0 and the 128K ROM, and Bad Demo took its interrupt into the 128K
        // editor's handler.
        requestInjection(ports);
      }
    });
    // Before init, because that is when the sound works out how big a frame of audio is: at the
    // speed that ships it comes to three samples a frame instead of the eight hundred odd a real
    // one has, and a replay is heard through that. Changing speed from there rebuilds it.
    speccy.speed.emulation = com.fpetrola.oozx.speccy.modules.timer.Speed.REAL_TIME;
    speccy.init();
    Path snapshot = snapshotFileOf(recording);
    Snapshots.of(speccy).load(snapshot.toAbsolutePath().toString());

    RzxSession session = new RzxSession(recording, speccy, playbackOf(speccy, player, recording), ports);
  ...
  public void rewind() {
    ...
    finished = false;
    Snapshots.of(speccy).load(snapshot.toAbsolutePath().toString());
    playback = playbackOf(speccy, player, recording);
  }
   ...
   * <p>
   * Once per recorded frame, because a recorded frame is now a frame of the machine: the driver
   * ends the machine's frame where the recording ended its own, so there is one picture and one
   * frame of sound for each, with the beeper's changes all inside it.
   *
  ...
  public void release() {
    ...
    speccy.scheduler.clear();
    speccy.zxClock.setTStates(0);
    // The timer's event as well as the frame's, in that order, the way selecting a machine does it.
    speccy.timer.addEvent();
    speccy.scheduler.schedule(speccy.machine.current.endOfFrame(), framesTStatesOf(speccy));
