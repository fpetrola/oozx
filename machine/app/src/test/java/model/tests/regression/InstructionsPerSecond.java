...
class InstructionsPerSecond extends MachineTest {
  ...
  void instructionsASecond() {
    for (String core : new String[]{"Generated", "OOP"}) {
      ...
      // A hundred frames is short enough that the two-byte counter cannot wrap, and long enough
      // to divide: the same program over the same frames does the same work every time.
