...
public class SnapshotZ80 implements SnapshotFile {
  ...
  /**
   * A page packed into the buffer, and how long that came out.
   * <p>
   * A page that does not get smaller is put in as it is and answered as 0x4000, which is how the
   * callers say "no compression". Random bytes come out LONGER than the page they came from -
   * every 0xED costs two - so packing them wrote past the end of the buffer and threw; and a page
   * that happened to pack to exactly 0x4000 was written as if it were the raw page, which the
   * next reader would have unpacked as bytes.
   */
  private int compressPageZ80(byte[] buffer, int page) {
    ...
    while (address < 0x4000) {
      if (addrDst + 4 > 0x4000) {
        for (int at = 0; at < 0x4000; at++) {
          buffer[at] = memory.readByte(page, at);
        }
        return 0x4000;
      }
      value = memory.readByte(page, address++);
      ...
      if (value == (byte) 0xED) {
        if (nReps == 0) {
          // A lone 0xED is always emitted literally, never as a run of one.
          buffer[addrDst++] = (byte) 0xED;
          buffer[addrDst++] = memory.readByte(page, address++);
        } else {
          // Repeated 0xED bytes are always run-length encoded, even below the usual 5-byte threshold.
          buffer[addrDst++] = (byte) 0xED;
          buffer[addrDst++] = (byte) 0xED;
        ...
        if (nReps < 4) {
          // Fewer than 5 equal bytes in a row are not worth run-length encoding.
  ...
  public byte[] saveToBytes(SpectrumState state) throws SnapshotException {
    spectrum = state;
  ...
  private void loadFromStream() throws IOException, SnapshotException {
    if (fIn.available() < 30) {
    ...
    spectrum.setTstates(WITHOUT_COUNTER);

    // A non-zero PC here is the .z80 v1.0 format's own signal, since v2/v3 store it as 0 and hold
    // the real PC in the extended header instead.
    if (z80.getRegPC() != 0) {
      ...
      if (hdrLen >= 26) {
        spectrum.setTstates(tstatesFrom(z80Header2, 23, spectrum.getSpectrumModel().tstatesFrame));
      }
  ...
  /**
   * What a header that carries no counter is given - a v1, or a v2 - and what this
   * machine got from it until it read the counter itself: 224 short of a 48K frame, whatever the
   * machine.
   */
  private static final int WITHOUT_COUNTER = 69664;

  /**
   * The counter a .z80 keeps in its extended header: which
   * quarter of the frame is running, and how much of that quarter is left to run.
   */
  private static int tstatesFrom(byte[] header, int at, int tstatesFrame) {
    int quarter = tstatesFrame / 4;
    int left = (header[at] & 0xff) | ((header[at + 1] & 0xff) << 8);
    int quarters = header[at + 2] & 0xff;
    return ((quarters + 1) % 4 + 1) * quarter - (left + 1);
  }

  private static void tstatesInto(byte[] header, int at, int tstates, int tstatesFrame) {
    int quarter = tstatesFrame / 4;
    int left = quarter - tstates % quarter - 1;
    header[at] = (byte) left;
    header[at + 1] = (byte) (left >>> 8);
    header[at + 2] = (byte) ((tstates / quarter + 3) % 4);
  }
  ...
  private void writeSnapshotData() throws IOException {
    ...
    // Bytes 6-7 (PC) stay 0: that value is what marks this as a v2/v3 header to the reader.
    ...
    z80HeaderV3[30] = 55; // length of the v3 extended header that follows
    z80HeaderV3[32] = (byte) z80.getRegPC();
    z80HeaderV3[33] = (byte) (z80.getRegPC() >>> 8);
    tstatesInto(z80HeaderV3, 55, spectrum.getTstates(), spectrum.getSpectrumModel().tstatesFrame);
    ...
    if (spectrum.isEnabledAY()) {
      // Bit 2 of byte 37 says a 48K has an AY fitted, which is the only machine it can say
      // anything about: a 128 or a +3 has one on the board. Setting it from "has an AY" put it on
      // every 128K snapshot, and the reader reads it back as a Fuller on a 48K.
      if (spectrum.isEnabledAYon48k()) {
        z80HeaderV3[37] |= 0x04;
      }
    ...
    int bufLen;
    if (spectrum.getSpectrumModel().codeModel == MachineTypes.CodeModel.SPECTRUM48K) {
      // A length of 0xffff marks the block as stored uncompressed.
      ...
      for (int page = 0; page < 8; page++) {
        // A page the snapshot never carried is not written back as a page of zeros: the file
        // would then claim to hold RAM it says nothing about, and it grows by a page for every
  ...
  // Only the v3 format is ever written, whatever version was loaded.
  @Override
