...
public class SnapshotSZX implements SnapshotFile {
    ...
    /** What wrote the file, from its creator block, and what it says about itself afterwards. */
    private String creator = "";
    private String creatorNote = "";

    /**
     * Whether this file has A and F the wrong way round.
     * <p>
     * libspectrum wrote them swapped until 0.5.0, so a file that says it came from one of those
     * has to be read the other way. libspectrum itself does this, reading the version out of the
     * text its creator block carries; without it, every szx those versions wrote reads with the
     * accumulator and the flags exchanged.
     */
    private boolean swapsAF() {
        java.util.regex.Matcher version =
            java.util.regex.Pattern.compile("libspectrum: (\\d+)\\.(\\d+)\\.(\\d+)").matcher(creatorNote);
        if (!version.find() || Integer.parseInt(version.group(1)) != 0) {
            return false;
        }
        int minor = Integer.parseInt(version.group(2));
        return minor < 5 || (minor == 5 && Integer.parseInt(version.group(3)) == 0);
    }
            ...
            while (fIn.available() > 0) {
                ...
                switch (szxId) {
                    case ZXSTBID_CREATOR:
                        ...
                        creator = new String(szCreator).trim();
                    ...
                    case ZXSTBID_Z80REGS:
                        ...
                        z80.setRegF(z80Regs[swapsAF() ? 1 : 0]);
                        z80.setRegA(z80Regs[swapsAF() ? 0 : 1]);
                        ...
                        z80.setRegFx(z80Regs[swapsAF() ? 9 : 8]);
                        z80.setRegAx(z80Regs[swapsAF() ? 8 : 9]);
                    ...
                    case ZXSTBID_ZXTAPE:
                        ...
                        if ((tape[2] & ZXSTTP_EMBEDDED) != 0) {
                            // The extension field is zero-padded, so the string stops at the first 0.
                            int nChars = 0;
                    ...
                    case ZXSTBID_IF1:
                        ...
                        break;
                    case ZXSTBID_MICRODRIVE: // never emulated: its design is unreliable enough to skip
                        while (szxLen > 0) {
                            szxLen -= fIn.skip(szxLen);
                        }
                        break;
