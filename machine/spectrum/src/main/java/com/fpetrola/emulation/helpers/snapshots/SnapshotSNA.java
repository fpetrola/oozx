...
public class SnapshotSNA implements SnapshotFile {
    ...
    public SpectrumState load(File filename) throws SnapshotException {
        ...
        try {
            ...
            if (snaLen == 49179) { // 48K
                // A 48K .sna keeps no PC: it is popped off the stack when the snapshot is
                // started, so the stack pointer has to point at RAM with room for the word.
                // One in ROM, or at the very top, has no PC to give, and the machine would run
                // from whatever the ROM happens to hold there.
                int sp = z80.getRegSP();
