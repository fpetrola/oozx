...
public class FuseResult {
  ...
  /** Only the events the processor under test reports are expected of it: one that reaches its memory itself reports no memory events. */
  public FuseResult(String testId, String registers, String state, String memory, List<Event> events, Set<String> reported) {
    this.registers = Arrays.stream(registers.split(" "))
        .filter(s -> !s.isEmpty())
        .mapToInt(s -> java.lang.Integer.parseInt(s, 16))
        .toArray();
    this.events = events.stream().filter(event -> reported.contains(event.getType())).collect(Collectors.toList());
    ...
    for (String memoryLine : memory.split("\n")) {
      if (!memoryLine.isBlank()) {
        ...
        for (int value : Arrays.copyOfRange(memoryData, 1, memoryData.length)) {
          int actual = cpu.getState().getMemory().peek(addr++);
