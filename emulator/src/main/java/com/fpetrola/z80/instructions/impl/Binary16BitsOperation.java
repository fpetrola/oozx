...
public class Binary16BitsOperation extends ParameterizedBinaryAluInstruction {
  ...
  /**
   * Packs bits 15/11 of each operand and 16/15/13/11 of the result into one lookup byte for the
   * flag table. Bit 13 of the result must only appear folded (for undocumented bit 5), not also
   * through the plain mask, or it aliases bit 11 of the second operand and corrupts half-carry.
   */
  ...
  protected void executeAction(int v1, int v2, int result) {
    // Zero flag uses the truncated 16-bit result, since a carry out of bit 15 (e.g. 0000+FFFF+1) must still read as zero in HL.
