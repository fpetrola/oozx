...
public class Cpi extends BlockInstruction {
  public static class CpiTableAluOperation extends AluOperation {
    ...
    protected int calculate2Values1Boolean(int value, int A, int BC) {
      // Third argument is BC-nonzero, not carry: CPI leaves carry untouched, but this table is
      // indexed by value/A/BC and has no room for it, so flagOperation restores it afterward.
      F = 0;
