...
public class In extends TargetSourceInstruction<ImmutableOpcodeReference> {
  ...
  /**
   * The 16-bit port address: a register source drives both halves, an immediate byte pairs with A
   * as the high half. MEMPTR is derived from this value, so callers must read it here rather than
   * recompute it.
   */
  public int port() {
