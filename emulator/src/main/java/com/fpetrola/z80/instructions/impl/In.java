...
public class In extends TargetSourceInstruction<ImmutableOpcodeReference> {
  ...
  /**
   * The sixteen lines the port is asked on: from a register the pair carries both halves, and from
   * a byte in the instruction the accumulator is the high one. Said here because it is the
   * instruction's own rule, and MEMPTR is one plus it - which is why this is asked for rather than
   * worked out a second time somewhere else.
   */
  public int port() {
    int port = source.read();
    return notRegister ? (port | a.read() << 8) & 0xFFFF : bc.read();
  }
