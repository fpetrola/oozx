...
/** The contention as a recording keeps it: one MC event per cycle, and the memory accesses beside them. */
public class RecordingPhaseProcessor extends PhaseProcessor {
  private final Consumer<Event> events;

  public RecordingPhaseProcessor(State state, Consumer<Event> events) {
    super(state);
    this.events = events;
  }
