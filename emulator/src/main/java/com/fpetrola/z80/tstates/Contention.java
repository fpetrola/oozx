...
/**
 * A group of the cycles an instruction spends on the bus beyond its memory accesses,
 * said as a value: at one moment of the execution, {@code times} cycles
...
public record Contention(Moment moment, int ordinal, Base base, int delta, int times, int tstates, Kind kind) {
  ...
  /** The bus cycle, by the name the recorded vectors use; the ULA contends them all alike, the machine's tests tell them apart. */
  public enum Kind {
    READ("readbyte"), WRITE("writebyte"), READ_NO_MREQ("contend_read_no_mreq"), WRITE_NO_MREQ("contend_write_no_mreq"),
    PORT_EARLY("contend_port_early"), PORT_LATE("contend_port_late");
