...
public final class SurfaceNetsBuilder {
  ...
  public static Model build(SpriteBitmap b, Sprite3DConfig cfg) {
    ...
    blur(f, nx, ny, nz, Math.max(0, Math.round(cfg.smoothing)));
      ...
      for (int y = 1; y < ny - 1; y++)
        for (int x = 1; x < nx - 1; x++) {
          ...
          for (int e = 0; e < 3; e++) {
            ...
            int c0 = cellIdx(x - du[0] - dv[0], y - du[1] - dv[1], z - du[2] - dv[2], nx, ny);
            int c1 = cellIdx(x - dv[0], y - dv[1], z - dv[2], nx, ny);
            int c2 = cellIdx(x, y, z, nx, ny);
            int c3 = cellIdx(x - du[0], y - du[1], z - du[2], nx, ny);
            ...
            } else { // flip so the winding always faces out of the solid
  ...
  private static int cellIdx(int x, int y, int z, int nx, int ny) {
