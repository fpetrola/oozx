...
//        System.out.println(String.format("Press keyCode = %d, modifiers = %d", key, evt.getModifiersEx()));
    ...
    /*
     * Windows never sends VK_ALT_GRAPH; instead it sends a Ctrl event followed by an Alt event
     * with both CTRL and ALT modifiers set, so the leading Ctrl wrongly presses Symbol-Shift and
     * must be undone here. Any other Alt press is ignored so OS keyboard shortcuts keep working
     * without leaking spurious keystrokes into the emulator. Some Windows keyboards send a right
     * Alt instead of AltGr, which is handled as the same case below.
     */
    if (winBug && key == KeyEvent.VK_ALT && (evt.getModifiersEx() == (InputEvent.CTRL_DOWN_MASK | InputEvent.ALT_DOWN_MASK)
      ...
      // On Windows, Alt+Control together usually means AltGr was pressed (reported by pastbytes).
...
//                    System.out.println(String.format("Key released #%d: %c\n", key, keychar));
    ...
    /*
     * Windows never sends VK_ALT_GRAPH; instead it sends Ctrl (with the Alt modifier already set)
     * followed by a plain Alt event. Any other Alt press is ignored so OS keyboard shortcuts keep
     * working without leaking spurious keystrokes into the emulator. Some Windows keyboards send a
     * right Alt instead of AltGr, which is handled as the same case below.
     */
