...
public class Z80 implements IZ80 {
   ...
   *
   * ADD HL,xx = Valor del registro H antes de la suma LD r,(IX/IY+d) = Byte
   * superior de la suma de IX/IY+d JR d = Byte superior de la dirección de
   * destino del salto
   *
   * 04/12/2008 No se vayan todavía, aún hay más. Con lo que se ha implementado
   * hasta ahora parece que funciona. El resto de la historia está contada en:
   * http://zx.pk.ru/attachment.php?attachmentid=2989
   *
   * 25/09/2009 Se ha completado la emulación de MEMPTR. A señalar que no se puede
   * comprobar si MEMPTR se ha emulado bien hasta que no se emula el
   * comportamiento del registro en las instrucciones CPI y CPD. Sin ello, todos
   * los tests de z80tests.tap fallarán aunque se haya emulado bien al registro en
   * TODAS las otras instrucciones. Shit yourself, little parrot.
   */
  private int memptr;

  /*
   * Algunos flags se precalculan para un tratamiento más rápido Concretamente,
   * SIGN, ZERO, los bits 3, 5, PARITY y ADDSUB: sz53n_addTable tiene el ADDSUB
   * flag a 0 y paridad sin calcular sz53pn_addTable tiene el ADDSUB flag a 0 y
   * paridad calculada sz53n_subTable tiene el ADDSUB flag a 1 y paridad sin
   * calcular sz53pn_subTable tiene el ADDSUB flag a 1 y paridad calculada El
   * resto de bits están a 0 en las cuatro tablas lo que es importante para muchas
   * operaciones que ponen ciertos flags a 0 por real decreto. Si lo ponen a 1 por
   * el mismo método basta con hacer un OR con la máscara correspondiente.
   */
  ...
  /*
   * Las funciones incRegXX y decRegXX están escritas pensando en que puedan
   * aprovechar el camino más corto aunque tengan un poco más de código (al menos
   * en bytecodes lo tienen)
   */
  ...
  public final Z80State getZ80State() {
    ...
    state.setIM(z80core.IntMode.values()[modeINT.ordinal()]);
  ...
  public final void setZ80State(Z80State state) {
    ...
    ffIFF1 = state.isIFF1();
    ffIFF2 = state.isIFF2();
    modeINT = IntMode.values()[ state.getIM().ordinal()];
...
//        this.state.registers.copyTo(com.fpetrola.z80.Z80.state.registers);
   ...
   * 0 y el resto se preservan. En cualquier caso, todo parece depender bastante
   * del modelo concreto de Z80, así que se escoge el comportamiento del modelo
  ...
  // r es igual a 0
  /*
   * En contra de lo que afirma el Z80-Undocumented, los bits 3 y 5 toman SIEMPRE
   * el valor de los bits correspondientes del valor a comparar para las
   * instrucciones BIT n,r. Para BIT n,(HL) toman el valor del registro escondido
   * (memptr), y para las BIT n, (IX/IY+n) toman el valor de los bits superiores
   * de la dirección indicada por IX/IY+n.
   *
  ...
  // Interrupción
  /*
   * Desglose de la interrupción, según el modo: IM0: M1: 7 T-Estados -> reconocer
   * INT y decSP M2: 3 T-Estados -> escribir byte alto y decSP M3: 3 T-Estados ->
   * escribir byte bajo y salto a N IM1: M1: 7 T-Estados -> reconocer INT y decSP
   * M2: 3 T-Estados -> escribir byte alto PC y decSP M3: 3 T-Estados -> escribir
   * byte bajo PC y PC=0x0038 IM2: M1: 7 T-Estados -> reconocer INT y decSP M2: 3
   * T-Estados -> escribir byte alto y decSP M3: 3 T-Estados -> escribir byte bajo
   * M4: 3 T-Estados -> leer byte bajo del vector de INT M5: 3 T-Estados -> leer
   * byte alto y saltar a la rutina de INT
   */
...
//        int tmp = tEstados; // peek8 modifica los tEstados
  ...
  /*
   * Desglose de ciclos de máquina y T-Estados M1: 5 T-Estados -> extraer opcode
   * (pá ná, es tontería) y decSP M2: 3 T-Estados -> escribe byte alto de PC y
   * decSP M3: 3 T-Estados -> escribe byte bajo de PC y PC=0x0066
   */
  private void nmi() {
  ...
  /*
   * Los tEstados transcurridos se calculan teniendo en cuenta el número de ciclos
   * de máquina reales que se ejecutan. Esa es la única forma de poder simular la
...
//            MemoryProxy.verifyChanges(state);
  ...
  private void compareFlags() {
    ...
    if (remoteF != localF)
      System.out.printf("no flag!: %H%n", getRegPC());
  }
...
//                System.out.println("Error instrucción CB " + Integer.toHexString(opCode));
  ...
  /*
   * Hay que tener en cuenta el manejo de secuencias códigos DD/FD que no hacen
   * nada. Según el apartado 3.7 del documento
   * [http://www.myquest.nl/z80undocumented/z80-documented-v0.91.pdf] secuencias
   * de códigos como FD DD 00 21 00 10 NOP NOP NOP LD HL,1000h activan IY con el
   * primer FD, IX con el segundo DD y vuelven al registro HL con el código NOP.
   * Es decir, si detrás del código DD/FD no viene una instrucción que maneje el
   * registro HL, el código DD/FD "se olvida" y hay que procesar la instrucción
   * como si nunca se hubiera visto el prefijo (salvo por los 4 t-estados que ha
   * costado). Naturalmente, en una serie repetida de DDFD no hay que comprobar
   * las interrupciones entre cada prefijo.
   */
...
//                System.out.println("Error instrucción DD/FD" + Integer.toHexString(opCode));
  ...
  private void decodeED() {
    ...
    case 0x47: { /* LD I,A */
      /*
       * El contended-tstate se produce con el contenido de I *antes* de ser copiado
       * el del registro A. Detalle importante.
       */
      MemIoImpl.contendedStates(getPairIR(), 1);
      regI = regA;
      break;
