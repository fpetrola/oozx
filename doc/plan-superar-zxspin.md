...
velocidad del emulador de referencia. ZXSpin corre un 20 % más rápido que él. Este documento dice dónde está el
...
`contend4x1` 3. La referencia hace exactamente estas cinco búsquedas (`contend_read_no_mreq` cinco veces
en el `.dat` de opcodes), así que acá hay una ventaja sobre ella, no solo paridad.
...
propias tablas (`tablesFor`, que estaba en `Machine`); `PhaseProcessorBase.contend` las usa; el
...
pregunta `zxClock.getTStates() < eventNextEvent` por instrucción. La referencia hace exactamente eso con
...
`overshoot` del timeout y lo que hace la referencia. `SpectrumZ80Clock` quedó en la suma que hereda de
...
- *`write`: una pregunta por página en vez de cinco.* Hoy cada escritura pregunta
  ...
  `Spec128.java:101`), dejan `if (page.screen && offset < 0x1b00)`. La referencia hace las cinco preguntas;
  esto también es ventaja sobre ella.
...
JSW redibuja los 768 atributos de la habitación cada frame: 6 144 `dirtyChunk` por frame. la referencia
hace el mismo bucle de ocho (`display_dirty64` → `display_dirty_chunk` × 8): paridad, y otra
ventaja posible sobre la referencia.
...
`dirtyXtable2/dirtyYtable2` se vuelven un shift y una máscara, como en la referencia.
...
siempre) y las celdas salen por shifts, como en la referencia; las cuatro tablas desaparecieron.
...
pasos faltan hasta el próximo (`quietSteps`) y los da de una vez; la referencia da los 2 216 de cada
...
y `step()` pregunta `beforeFetch.armed() || afterInstruction.armed()`. La referencia pregunta una cosa por
...
la regla anterior — el frame de audio se dimensiona para la velocidad pedida:
...
velocidad pedida, como en la referencia, así que a 300 % el juego se oye tres veces más agudo — acelerado,
