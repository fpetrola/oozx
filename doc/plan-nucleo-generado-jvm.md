...
Una contención (`PhaseProcessorBase.contend`): búsqueda de página, y `times` veces
...
`PhaseProcessorBase.contend` es exactamente lo que hay que inlinear: `if (page.contended) times ×
...
El núcleo puro (`GeneratedZ80`, ganchos abstractos) sigue existiendo: es lo que el emulador de referencia verifica
...
`PhaseProcessorBase.contend`, `Ula.addUlaStates` y `SpectrumZ80Clock.addTStates`, unas cien líneas
...
1. **`machine/core` pasa a ser entrada del generador.** Es el mismo contrato que ya rige para las
   ...
   pasando la referencia, que no conoce la máquina.
...
- **M1. El timeout del reloj deja de ser un `Consumer`.** `SpectrumZ80Clock.setTimeout` instala
  ...
  y la interrupción entre instrucciones, igual que el emulador de referencia. Lo segundo cambia la granularidad del
...
1. **A3 y A2** en `Folder` y `Simplifier`: sin riesgo semántico, cambian el bytecode. Gate: la batería de vectores de CPU
   (1355), ALU, `IsCurrentTest` regenerado; el tamaño de bytecode por método, antes y después.
   *Hecho.*
2. **A4**: el tejido por rama. Gate: el emulador de referencia evento por evento, que es exactamente lo que cambia.
   *Hecho.*
...
Gate: `emulator` 3003 tests verdes, el emulador de referencia evento por evento sobre los dos núcleos, la ALU
...
- **Semántica**: la batería de vectores de CPU (1355) evento por evento sobre `GeneratedZ80`; `GeneratedAluReferenceTest`;
...
- **Dos artefactos generados.** El núcleo de la máquina acopla el generado a `oozx.Memory`,
  ...
  pagarlo. Si se paga, está contenido: el puro sigue siendo el que el emulador de referencia verifica, y el de la
...
- **La frontera mal puesta es un bug silencioso.** Congelar `contended` o copiar `tStates` da un
  núcleo que pasa la referencia —que no pagina— y falla en un juego de 128K media hora después. El test de
