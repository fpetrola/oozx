...
Medido sobre `emulator` el 1 de septiembre de 2026, con la batería de vectores de CPU en 1355 verdes.

**Estado (2 de septiembre de 2026, rama `nucleo-generado`): hecho, salvo C.** A, B y D están
hechos y commiteados; el generador existe y `GeneratedZ80.java` pasa los 1355 tests de la batería de vectores de CPU
...
3. **El núcleo generado es exactamente tan correcto como el OOP** con los aspectos que se le
   tejieron. Por eso el gate es el mismo: los 1355 tests de la batería de vectores de CPU, que comparan registros,
...
| contención (aspecto) | `z80/tstates/PhaseProcessor` (417) + base, `CachedPhase`, 4 fases, 4 visitors | lambdas registradas por instancia; 8 ganchos abstractos que la máquina (`PhaseProcessor`) y el test (`RecordingPhaseProcessor`) implementan |
| MEMPTR (aspecto) | `cpu/MemptrUpdater` (205) + `spy/MemptrUpdateInstructionSpy` | lo instalan los tests de la batería de vectores de CPU y el translator; la máquina hoy no, y ahí MEMPTR lo escriben sólo los hooks de `Ld`/`Ex` que viven en `PhaseProcessor`, y `doInt` |
...
- **Oráculos**, todos existentes: la batería de vectores de CPU (1355, evento por evento), `AluReferenceTest` (todas las
...
la batería de vectores de CPU en verde, y cada una le quita al generador una regla que de otro modo tendría que nombrar
algo del modelo. Son propuestas de forma; el gusto final es del dueño del modelo.

### A. Los aspectos dejan de estar en el modelo

Hoy el modelo conoce a la contención. Cinco archivos del núcleo importan `com.fpetrola.z80.tstates`:
...
un `RecordingPhaseProcessor`** —el procesador de contención de los vectores de CPU— y lo aplica en
...
tiene `events`/`addEvent` con `Event`, que es el registro de eventos del arnés de los vectores: lo usan
`RecordingPhaseProcessor`, `AddStatesIO` y `FuseResult`, nadie más.
...
- `Instruction`, `AbstractInstruction` y `DummyInstruction` pierden `CachedPhase`; el fetcher
  ...
  `opcodes` o `registers` importa `com.fpetrola.z80.tstates`.
...
- **Un solo gancho**, con argumentos explícitos: `contend(address, times, tstates)`. Los ocho
  ...
  con la dirección ya calculada; `PhaseProcessor` queda en cinco líneas (página contendida →
  `Ula.addUlaStates` por vez; si no, el reloj) y `RecordingPhaseProcessor` en tres (un `Event`
  MC por vez).
...
3. **El fetcher lee los desplazamientos de índice al preparar la instrucción; los inmediatos y
   ...
   lo que el emulador de referencia espera y el código ya hace: un salto relativo lee `d` sólo si salta (`20_2`: un MC
...
Neto estimado: ≈ −400 líneas. Gate: la batería de vectores de CPU evento por evento más los 338 de `machine`.
...
Z80Core, un salto no tomado no lo toca; la batería de vectores de CPU no distingue porque todos sus tests arrancan con
...
vueltos variables). Coinciden con lo que el emulador de referencia espera: para `20_1` (tomado) un MR en 0001 y cinco
...
frames) y sobre un programa sintético en `MockedMemory`. Baseline hoy: la batería de vectores de CPU 1355/1355.

**1. A: los aspectos fuera del modelo. HECHO** (`49de2b1d3`, −73 líneas). Gate: la batería de vectores de CPU, `machine`, y un grep: ningún archivo de
`cpu`, `instructions`, `opcodes` ni `registers` importa `com.fpetrola.z80.tstates`.

**2. B: la contención como valores. HECHO** (`24775046d`, −537 líneas). Gate: la batería de vectores de CPU evento por evento, `ZXSpectrumContendedMemoryTests`,
...
**3. D: MEMPTR con un dueño. HECHO** (`f395170d5`, −50 líneas). Gate: la batería de vectores de CPU verifica MEMPTR en cada test; el translator y los tests
...
leída una vez; los desplazamientos de índice al preparar; el prefijo que declara. La batería de vectores de CPU después de
cada uno. Hasta entonces el generador lleva las dos reglas que quedan pendientes.
...
`PhaseProcessor` a ojo y contra la batería de vectores de CPU después.
...
eventos. Éste es el paso donde todo se cobra: si un `case` sale mal, la batería de vectores de CPU dice cuál y el dorado
dice por qué.
...
adentro— y le implementa `contend` con lo que hoy hace `PhaseProcessor`. Con el generado:
...
|---|---|---|
| la batería de vectores de CPU, 1355 tests | registros, memoria, MEMPTR, T-states, **cada evento de bus con su instante** | los dos núcleos, desde el paso 7 |
...
| grep de imports | que el modelo no conozca `com.fpetrola.z80.tstates` | build, desde el paso 1 |
...
- **B cambia la forma de un aspecto que hoy pasa 1355 tests**: 35 reglas transcriptas a valores.
  La batería de vectores de CPU las verifica evento por evento y `ZXSpectrumContendedMemoryTests` en la máquina; se hace
...
- **JavaParser sin resolución de símbolos**: renombres de locales, sombras, `this` implícito. Se
  contiene con los dorados y con la batería de vectores de CPU; si un caso lo pide, se agrega el symbol solver.
...
- **El orden de las lecturas.** Es la parte con más trucos acumulados; La batería de vectores de CPU los verifica evento
...
- **`UnrolledRegisterBank` lleva tiempo sin usarse** y no enmascara al escribir (`A = value`).
  Antes de apoyar el generado en él, correr la batería de vectores de CPU con el núcleo OOP sobre ese banco: una línea en
...
| La batería de vectores de CPU sobre el generado | 1355/1355, evento por evento (`GeneratedFuseTests`) |
