...
| `export [salida.json]` | TODO lo anterior consolidado en un JSON legible por máquina (`analysis/game-model.json`): zonas de memoria con tipo, cada sprite con dirección/tamaño/uso, fuente, estructura de entidades con campos y fórmulas, **estructuras/variant-records por rutina** (la salida de `structs` como datos), variables con sus rangos de valores, rutinas clasificadas por evidencia, episodios | el "modelo del juego" para consumir desde otras herramientas |
| `structs [metodo]` | recuperación de estructuras: parsea los offsets `mem[IX+k]` de las ecuaciones, deduce base/stride/elementos de las direcciones y bits observados, etiqueta cada campo (coordenada, tipo/flag por edges COND, selector de gráfico, índice) y descubre los **variant records**: por cada branch cuya condición viene de un campo, qué campos usa exclusivamente cada rama (ej. `$37056`: `+0&3==1` → rama vertical usa +1/+3/+4, rama horizontal usa +2) | "¿qué struct maneja esta rutina y cómo la recorre como arreglo?" |
